...
class calcular{
    ...
    public function processReport( $tipoReporte, $clave)
    {
        ...
        while ($registro = mysql_fetch_array($result, MYSQLI_ASSOC))
        {
            ...
            if($materia != NULL)
            {
                ...
                if($row['existe'] != NULL)
                {
                    ...
                    */
                    ...
                    $sql="SELECT personal.percve, personal.depcve, departamento.depcve, departamento.depnom FROM personal INNER JOIN departamento ON personal.depcve = departamento.depcve and personal.percve = '$clave'";
                    $query=mysql_query($sql,$enlace);
                    $row=mysql_fetch_assoc($query);

                    $promedio[0][34]= $row['depnom'];
            ...
            }//cierre del if que dice que si el profesor tiene materias
