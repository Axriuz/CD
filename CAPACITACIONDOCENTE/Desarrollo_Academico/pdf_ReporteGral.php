...
$con=mysqlii_connect("$host","$user","$pass","$db_name");
...
$result= mysqli_query($con,"SELECT COUNT(*) total FROM curso  where  Periodo='$p' and year(CursoInicio)='$anio'");
$total = mysqli_fetch_row($result);

$totalhoras=$total[0]*30;

$result2= mysqli_query($con,"SELECT COUNT(*) from maestro inner join curso on maestro.Emp=curso.Instructor and curso.Periodo='$p' and year(CursoInicio)='$anio' and maestro.sexo='Femenino'");
$total2 = mysqli_fetch_row($result2);
//ECHO $total2[0];

$result3= mysqli_query($con,"SELECT COUNT(*) from maestro inner join curso on maestro.Emp=curso.Instructor and curso.Periodo='$p' and year(CursoInicio)='$anio' and maestro.sexo='Masculino'");
$total3 = mysqli_fetch_row($result3);
//ECHO $total3[0]; 

//Consulta que retorna la cantidad de mujeres que tomaron cursos en el periodo seleccionado 
$sqlsexF=mysqli_query($con,"SELECT COUNT(DISTINCT maestro.Nombre,maestro.ApellidoP) FROM maestro inner join matriculas on maestro.Emp=matriculas.Emp inner join curso on matriculas.curso=curso.nombre where curso.Periodo='$p' and Year(curso.CursoInicio)='$anio' and maestro.sexo='Femenino'");
$totalF = mysqli_fetch_row($sqlsexF);
//echo $totalF[0];

//Consulta que retorna la cantidad de hombres que tomaron cursos en el periodo seleccionado 
$sqlsexM=mysqli_query($con,"SELECT COUNT(DISTINCT maestro.Nombre,maestro.ApellidoP) FROM maestro inner join matriculas on maestro.Emp=matriculas.Emp inner join curso on matriculas.curso=curso.nombre where curso.Periodo='$p' and Year(curso.CursoInicio)='$anio' and maestro.sexo='Masculino'");
$totalM = mysqli_fetch_row($sqlsexM);
//echo $totalM[0];
   
  
$result4=mysqli_query($con,"SELECT COUNT(DISTINCT(emp)) from matriculas inner join curso on matriculas.Curso=curso.nombre and curso.Periodo='$p' and year(CursoInicio)='$anio'");
