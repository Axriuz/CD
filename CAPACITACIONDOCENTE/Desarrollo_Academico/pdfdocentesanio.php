...
$sql="SELECT m.ApellidoP,m.ApellidoM,m.nombre, count(ma.emp) FROM matriculas ma INNER JOIN maestro m ON ma.emp = m.emp INNER JOIN curso on curso.Nombre=ma.Curso WHERE year(curso.CursoInicio)='".$anio."' and m.area = '$curso' GROUP BY ma.emp HAVING count( ma.emp ) >=0  ORDER BY m.ApellidoP "; 
